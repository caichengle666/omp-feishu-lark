...
import { compareVersions, resolveTargetVersion } from "./upgrade.js";
...
async function upgradeDaemon(targetVersion: string): Promise<string> {
    const current = pluginVersion();
    let target: string;
    {
      let latest: string | undefined;
      if (!targetVersion) {
        // 显式查 registry latest，绕开 bun 的 dist-tag 缓存坑
        const http = await fetch("https://registry.npmjs.org/@caichengle/omp-feishu-lark/latest", { signal: AbortSignal.timeout(15_000) });
        if (!http.ok) throw new Error(`无法查询 npm registry：HTTP ${http.status}`);
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (cmd === "upgrade") {
          ...
          try {
            const report = await upgradeDaemon(targetVersion || "");
...
async function runProcess(
  ...
  return await new Promise<{ code: number | null; stdout: string; stderr: string }>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`升级安装超过 ${Math.ceil(options.timeout / 1000)} 秒，已终止安装子进程。`));
    }, options.timeout);
