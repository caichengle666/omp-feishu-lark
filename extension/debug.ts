...
async function writeBatch(lines: string[]) {
  try {
    ...
    try { await chmod(DEBUG_LOG_PATH, 0o600); } catch {}
...
function truncate(value: unknown, key?: string): unknown {
  if (key && /(?:authorization|content|password|prompt|secret|text|token|output)/i.test(key)) return "[redacted]";
  if (typeof value === "string") return value.length > MAX_VALUE_LENGTH ? `${value.slice(0, MAX_VALUE_LENGTH)}...` : value;
  if (Array.isArray(value)) return value.map((item) => truncate(item, key));
  if (value && typeof value === "object") {
    const out: Record<string, unknown> = {};
    for (const [entryKey, item] of Object.entries(value)) out[entryKey] = truncate(item, entryKey);
    return out;
  }
  return value;
}
