...
export class FeishuCardActionWebhook {
  ...
  async start() {
    ...
    const dispatcher = new lark.CardActionHandler(
      {
        verificationToken: this.config.cardActionToken,
      },
      async (data: any) => {
      let action;
      try {
    ...
    this.server.on("request", (req, res) => {
      ...
      // 自有的 Bearer 令牌校验：飞书 SDK 的 verificationToken 只校验 v1 卡片/加密事件，
      // 对 v2 schema 卡片回调不生效，因此这里在进入 dispatcher 前强制校验。
      const token = this.config.cardActionToken;
      if (token) {
        ...
        const valid = suppliedBuf.length === expectedBuf.length && timingSafeEqual(suppliedBuf, expectedBuf);
        if (!valid) {
          res.statusCode = 401;
