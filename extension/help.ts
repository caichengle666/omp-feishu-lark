...
export function feishuHelpText(ompCommands?: ReadonlyArray<OmpCommandInfo>) {
  const lines = [
    ...
    "/feishu upgrade - 管理员安装最新版；指定版本可升级或降级（例如：/feishu upgrade 0.4.xx）",
    ...
    "/feishu provider remove <名称> confirm - 删除 Provider（管理员），例如：/feishu provider remove openai confirm",
    "/feishu commands 或 /commands - 查看 OMP 原生命令（部分已可在飞书直接执行，其余需在本机终端运行）",
    "/new - 新建当前飞书会话（群聊需管理员）",
    ...
    "/compact - 手动压缩当前飞书会话的上下文，可附带压缩重点，例如：/compact 保留接口参数",
    "/autocompact - 查看或开启/关闭自动上下文压缩，如 /autocompact on（群聊需管理员）",
    "/stop - 停止当前聊天正在执行的任务（群聊需管理员）",
    "/workspace PATH - 切换当前聊天的工作目录（管理员），例如：/workspace /srv/project",
    "/send PATH - 发送当前工作区内的文件或图片（管理员），例如：/send report.pdf",
