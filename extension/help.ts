...
export function feishuHelpText(ompCommands?: ReadonlyArray<OmpCommandInfo>) {
  const lines = [
    ...
    "/feishu gateway list - 查看已配置网关（管理员）",
    "/feishu gateway add <名称> <baseUrl> <API Key> [api] [modelId...] - 添加或更新网关（管理员）",
    "/feishu gateway test <名称> - 测试网关或检查静态模型（管理员）",
    "/feishu gateway remove <名称> confirm - 删除网关（管理员）",
