...
test("bot parser recognizes provider management commands", () => {
  assert.deepEqual(parseBotCommand("/feishu provider"), { name: "providerList" });
  assert.deepEqual(parseBotCommand("/feishu provider list"), { name: "providerList" });
  assert.deepEqual(parseBotCommand("/feishu provider add edge https://api.example.test/v1 secret-key openai-completions"), {
    name: "providerAdd",
...
test("provider management requires an administrator and forwards parsed arguments", async () => {
  ...
  const providerMessage = { ...message, senderOpenId: "ou_user" };
  const denied = new FeishuMessageHandler({}, () => transport, undefined, { isAdmin: () => false });
  assert.equal(await denied.handleCommand(providerMessage, "p2p:ou_user", "/feishu provider list"), true);
  ...
  for (const command of [
    "/feishu provider list",
    "/feishu provider add edge https://api.example.test secret",
    "/feishu provider test edge",
    "/feishu provider remove edge confirm",
  ]) {
    assert.equal(await allowed.handleCommand(providerMessage, "p2p:ou_user", command), true);
  }
