...
test("daemon host session completes a Feishu prompt from agent_end", async () => {
  let injected;
  const host = {
    sendUserMessage: (content) => { injected = content; },
    setModel: async () => true,
    getModel: () => ({ provider: "oc2", id: "deepseek-v4-flash-free" }),
    abort: () => {},
  };
  const manager = new ConversationManager(process.cwd(), undefined, {}, host);
  const replies = [];
  const pending = manager.prompt("chat", "hello", async (text) => replies.push(text));
  await new Promise((resolve) => setTimeout(resolve, 0));

  assert.equal(injected, "hello");
  manager.handleHostEvent({
    type: "agent_end",
    messages: [{ role: "assistant", content: [{ type: "text", text: "HOST_OK" }] }],
  });
  await pending;

  assert.deepEqual(replies, ["HOST_OK"]);
});

test("daemon host session stop aborts the OMP host turn", async () => {
  let aborted = 0;
  const host = {
    sendUserMessage: () => {},
    setModel: async () => true,
  ...
  const manager = new ConversationManager(process.cwd(), undefined, {}, host);
  const prompt = manager.prompt("chat", "hello", async () => {});
  await new Promise((resolve) => setTimeout(resolve, 0));
  const result = await manager.stopConversation("chat", async () => {});

  assert.equal(result.status, "stopped");
  assert.equal(aborted, 1);
  manager.handleHostEvent({ type: "agent_end", messages: [] });
  await prompt;
});
