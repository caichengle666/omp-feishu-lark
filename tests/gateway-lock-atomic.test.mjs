...
test("gateway locks.json is written atomically and recovers from corruption", () => {
  ...
  const script = [
    ...
    '  await first.handle.update("connected");',
    '  if (first.handle.owner.status !== "connected") process.exit(7);',
    '  const locksPath = join(root, "locks.json");',
    '  if (JSON.parse(readFileSync(locksPath, "utf8"))[first.handle.owner.key]?.status !== "connected") process.exit(8);',
